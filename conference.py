...
class ConferenceApi(remote.Service):
    ...
    def getConferenceSessions(self, request):
        """returns all sessions related the conference in request."""
        # check if conf exists given websafeConfKey
        # get conference; check that it exists
        wsck = request.websafeConferenceKey
        c_key = ndb.Key(urlsafe=wsck)
    ...
    def _createSessionObject(self, request):
        ...
        # check if the user is the organizer of the conf
        ...
        # generate Session key from the id
    ...
    def _sessionInWishlist(self, request, add=True):
        """add or delete session for user's wishlist."""
        retval = False
        # preload necessary data items
        user = endpoints.get_current_user()
        if not user:
    ...
    def deleteSessionInWishlist(self, request):
        ...
        return self._sessionInWishlist(request, False)
